...
function createJobQueue() {
  ...
  addJob = function(cb) {
    ...
    //It should return a promise that resolves with the value returned by job, whenever job ends up getting executed.
    ...
    taskQueue.push(jobPromise);
    return jobPromise;
  };

  cancelJob = function(job) {
    // cancelJob removes a job from the queue.
    // This should reject the promise returned by addJob.
    // If no matching job is found, it does nothing.
  };
  processAllJobs = function() {
    // When processAllJobs is called, the queue should process jobs(by invoking them)
    // one - at - a - time in FIFO order until there are none left,
    // then resolve with the number of jobs successfully processed(i.e., that did not reject or throw an error).
    // If any job cannot be processed--
    // because job rejects or throws an error when invoked--
    // the promise returned by addJob should be rejected.However,
    // this should not stop processAllJobs from processing the rest of the queue.
  };
}
